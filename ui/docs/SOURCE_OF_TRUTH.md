...
- **`themeMode` never overrides legibility** — `paintAppearance()` picks text colours from the light/dark mode but derives the page background from `palette.background`/`palette.surface`. An explicit `dark`/`light` mode skips the contrast inference, so a generated theme pairing a mode with a palette from the opposite end used to render near-white text on a near-white page. `toThemeSide()` now pulls the two base colours onto the side the mode asked for before any gradient is derived; palettes that already agree with the mode pass through untouched, so hand-picked themes and the shipped defaults are unaffected.
